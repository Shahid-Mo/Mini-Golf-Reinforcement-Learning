...
BALL_RADIUS = 10
HOLE_RADIUS = 15
FRICTION = 0.9  # Increased from 10 to 20
ELASTICITY = 0.8
VELOCITY_THRESHOLD = 1  # Define a threshold for stopping the ball
...
class Ball:
    ...
    def hit(self, power, angle):
        ...
        if self.body.velocity.length < 1 and not self.in_hole:
            impulse = power * pymunk.Vec2d(math.cos(angle), math.sin(angle))
            self.body.apply_impulse_at_local_point(impulse)
    
    def stop_if_slow(self, velocity_threshold=VELOCITY_THRESHOLD):
        """Stop the ball completely if it's moving slower than the threshold."""
        if self.body.velocity.length < velocity_threshold and not self.in_hole:
            # Set velocity to zero
            self.body.velocity = pymunk.Vec2d(0, 0)
            # Also eliminate any rotational velocity
            self.body.angular_velocity = 0
    
    @property
    def is_moving(self):
        # Check if the ball is moving significantly
        return self.body.velocity.length > VELOCITY_THRESHOLD and not self.in_hole
...
class MinigolfEnv(gym.Env):
    ...
    def step(self, action):
        ...
        for _ in range(10):  # Adjust this number for physics accuracy vs. performance
            ...
            # Apply the stop condition after each physics step
            self.ball.stop_if_slow()
